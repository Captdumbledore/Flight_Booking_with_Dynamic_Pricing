...
# Amadeus API Configuration - Load from .env
...
# Validate Amadeus credentials on startup
if not AMADEUS_CLIENT_ID or not AMADEUS_CLIENT_SECRET:
    print("\n" + "="*60)
    print("⚠️  WARNING: Amadeus API credentials not configured!")
    print("="*60)
...
async def get_amadeus_token():
    ...
    try:
        ...
        if not AMADEUS_CLIENT_ID or not AMADEUS_CLIENT_SECRET:
            print("⚠️ Amadeus credentials not configured in .env file")
            return None
...
async def load_real_flight_data():
    ...
    try:
        ...
        POPULAR_ROUTES = [
            ("JFK", "LAX"), ("LHR", "JFK"), ("SFO", "JFK"),
            ("LAX", "ORD"), ("MIA", "JFK"), ("DFW", "LAS"),
            ("ATL", "MCO"), ("BOS", "CHI"), ("SEA", "SFO"),
            ("DEN", "PHX"), ("CDG", "LHR"), ("DXB", "LHR"),
            ("SIN", "HKG"), ("NRT", "ICN"), ("SYD", "MEL"),
        ]
...
def generate_booking_pdf(booking: dict) -> str:
    ...
    y = draw_content_row("Route:", f"{booking['origin']} → {booking['destination']}", y)
...
def get_all_bookings():
    ...
    try:
        ...
        if not bookings_data:
            ...
            return {"total_bookings": 0, "bookings": []}
